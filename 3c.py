...
if __name__ == "__main__":
    ...
    csvFile = pandas.read_csv("iris.csv").dropna()
    all_data = [[row[0], row[1], row[2], row[3], row[4]] for i, row in csvFile.iterrows()]
    random.shuffle(all_data)

    # Klase pretvaramo u numericke vrednosti
    label_encoder = LabelEncoder()
    all_labels = [row[4] for row in all_data]
    ...
    train_X = [[row[0], row[1], row[2], row[3]] for row in train_data]
    train_Y = numeric_labels[:split_index]
    test_X = [[row[0], row[1], row[2], row[3]] for row in test_data]
    ...
    # U odnosu na prethodni grafik (model), tacnost ovog je dosta veca i stabilnija
    # dodavanjem 2 feature-a podatci su precizniji i sum se smanjio
    # Prvi model je dosta osetljiviji sto se tice odabira k vrednosti za razliku od ovog,
    # gde tacnost ne varira mnogo